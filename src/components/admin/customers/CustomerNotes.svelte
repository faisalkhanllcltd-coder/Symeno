...
<div class="flex h-full flex-col border border-white/10 bg-[#111318]">
  ...
  <div class="min-h-[200px] flex-1 space-y-4 overflow-y-auto p-4">
    {#each notes as note}
      <div class="border-l-2 border-[#36f4a4] bg-white/5 p-3">
        <div
  ...
  <form onsubmit={addNote} class="border-t border-white/10 bg-white/[0.02] p-4">
    <div class="flex gap-2">
      <input
        ...
        class="flex-1 border border-white/10 bg-[#1A1D23] px-3 py-2 font-mono text-xs text-white focus:border-[#36f4a4]/50 focus:outline-none"
      />
      <button
        type="submit"
        disabled={isSubmitting}
        class="bg-[#36f4a4] px-4 py-2 text-[10px] font-bold tracking-widest text-black uppercase transition-colors focus:outline-none disabled:opacity-50"
        >Log</button
      >
    </div>
  </form>
