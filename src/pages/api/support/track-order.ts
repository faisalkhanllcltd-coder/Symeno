...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!order_id || !email) throw new Error('Order ID and Email are strictly required.');
    ...
    if (!order) {
      ...
      return new Response(JSON.stringify({ error: 'No matching records found for this combination.' }), { status: 404 });
    }
