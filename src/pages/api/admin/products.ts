...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const { success } = await env.DB.prepare(
      ...
      crypto.randomUUID(), data.title, data.slug, data.base_price || 0, 
      data.description || '', data.stock_status || 'IN_STOCK', 1
    ).run();
...
export const DELETE: APIRoute = async ({ request, locals }) => {
  try {
    ...
    await env.DB.batch([
      env.DB.prepare('DELETE FROM product_variants WHERE product_id = ?1').bind(id),
