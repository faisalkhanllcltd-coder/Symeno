...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const [revenueRes, ordersRes, productsRes, customersRes] = await db.batch([
      ...
      db.prepare("SELECT DATE(created_at) as date, COUNT(*) as count FROM users GROUP BY DATE(created_at) ORDER BY date DESC LIMIT 30")
    ]);
    ...
    return new Response(JSON.stringify({ error: 'Analytics Aggregation Failed' }), { status: 500 });
  }
};