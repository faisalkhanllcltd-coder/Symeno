...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const { variant_id, adjustment, reason, notes } =
    ...
    const current = await db
      .prepare('SELECT inventory_quantity FROM product_variants WHERE id = ?1')
      .bind(variant_id)
      .first();
    ...
    return new Response(JSON.stringify({ success: true, newStock }), {
      status: 200,
    });
  } catch (e: any) {
