...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    await env.DB.prepare(
      ...
      crypto.randomUUID(), data.code.toUpperCase(), data.type, data.value,
      data.minimum_order || 0, data.usage_limit || null,
      data.start_date || null, data.end_date || null, data.stackable ? 1 : 0
    ).run();
