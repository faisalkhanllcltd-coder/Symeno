...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    enforceAdmin(locals);
    const { results } = await env.DB.prepare('SELECT * FROM brands WHERE id = ?1').bind(params.id).all();
...
export const PUT: APIRoute = async ({ request, params, locals }) => {
  try {
    ...
    await env.DB.prepare(
      ...
      data.name, data.slug, data.description, data.logo_url, data.cover_url,
      data.country, data.website_url, data.is_featured ? 1 : 0,
      data.seo_title, data.seo_description, params.id
    ).run();
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  try {
    ...
    await env.DB.prepare('DELETE FROM brands WHERE id = ?1').bind(params.id).run();
    return new Response(JSON.stringify({ success: true }), { status: 200 });
  } catch (e) {
