...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    await env.DB.prepare(
      ...
      crypto.randomUUID(), data.name, data.slug, data.description || '', 
      data.logo_url || null, data.cover_url || null, data.country || null, 
      data.website_url || null, data.is_featured ? 1 : 0, 
