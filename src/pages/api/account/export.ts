...
export const GET: APIRoute = async ({ locals }) => {
  try {
    ...
    const [userReq, ordersReq] = await env.DB.batch([
      ...
      env.DB.prepare("SELECT id, total, status, created_at FROM orders WHERE customer_id = ?1").bind(locals.user.id)
    ]);
    ...
    return new Response(JSON.stringify(exportData, null, 2), { 
      ...
      headers: { 
        ...
        'Content-Disposition': 'attachment; filename="symeno_personal_data_export.json"'
      }
    });
  } catch (e) {
