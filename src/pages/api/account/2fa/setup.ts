...
export const POST: APIRoute = async ({ locals }) => {
  try {
    ...
    const uri = createTOTPKeyURI("Symeno Operator Panel", locals.user.email, key);
    
    // 3. Store the temporary key in the database (requires a 'totp_secret' column in users)
    const secretBase64 = encodeBase64(key);
    
    await env.DB.prepare("UPDATE users SET totp_secret = ?1 WHERE id = ?2").bind(secretBase64, locals.user.id).run();
    ...
    return new Response(JSON.stringify({ error: '2FA generation failed' }), { status: 500 });
  }
};