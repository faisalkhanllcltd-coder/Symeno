...
export const PUT: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (action === 'MARK_ALL_READ') {
      await db.prepare("UPDATE customer_notifications SET is_read = 1 WHERE customer_id = ?1 AND is_read = 0")
        .bind(locals.user.id).run();
    } else if (id) {
      // Mark specific notification
      await db.prepare("UPDATE customer_notifications SET is_read = 1 WHERE id = ?1 AND customer_id = ?2")
