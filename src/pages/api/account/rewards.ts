...
export const GET: APIRoute = async ({ locals }) => {
  try {
    ...
    const [balanceReq, historyReq] = await db.batch([
      ...
      `).bind(locals.user.id),
      db.prepare("SELECT * FROM points_ledger WHERE user_id = ?1 ORDER BY created_at DESC LIMIT 20").bind(locals.user.id)
    ]);
    ...
    return new Response(JSON.stringify({ stats, history: historyReq.results }), { status: 200 });
  } catch (e) {
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const { results } = await db.prepare("SELECT SUM(points) as balance FROM points_ledger WHERE user_id = ?1").bind(locals.user.id).all();
    const currentBalance = results[0]?.balance || 0;
    
    if (currentBalance < points) throw new Error('Insufficient points balance.');
    ...
    await db.batch([
      ...
      `).bind(crypto.randomUUID(), locals.user.id, -Math.abs(points), `Redeemed for $${discountValue} discount`),
    ...
    return new Response(JSON.stringify({ success: true, discount_code: code, value: discountValue }), { status: 201 });
  } catch (e: any) {
