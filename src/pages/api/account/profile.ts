...
export const PUT: APIRoute = async ({ request, locals }) => {
  try {
    if (!locals.user) throw new Error('UNAUTHENTICATED');
    const { first_name, last_name, phone, dob, gender, avatar_url, preferences } = await request.json() as any;
    ...
    await env.DB.prepare(`
      ...
      WHERE id = ?8
    `).bind(
      first_name, last_name, phone, dob, gender, avatar_url, 
      JSON.stringify(preferences || {}), locals.user.id
    ).run();
