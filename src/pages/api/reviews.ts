...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (locals.user) {
      ...
      `).bind(customerId, data.product_id).first();
      
      if (hasPurchased) isVerified = 1;
    } else {
      // Must be logged in to review in the $10M standard to prevent spam
      return new Response(JSON.stringify({ error: 'Authentication required to submit trusted reviews.' }), { status: 401 });
    }

    await db.prepare(`
      INSERT INTO reviews (id, product_id, customer_id, rating, title, comment, is_verified_buyer, status)
      VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, 'PENDING')
    `).bind(crypto.randomUUID(), data.product_id, customerId, data.rating, data.title, data.comment, isVerified).run();

    return new Response(JSON.stringify({ success: true, verified: isVerified === 1 }), { status: 201 });
  } catch (e: any) {
